...
32. 时间流形态的**装箱与织造**（v1.13）：`sessions ≥ 1` ∧ `sessions ≤ Σsequences ≤ 2 × sessions`（交叉并发度恒 k ∈ {1,2}）；`duplicates ∈ [0, Σsequences]`；`noise_ratio ∈ [0,1)`（> 0 ⇒ `noise_instruction` 非空）；`frame_gap_s`：默认 v1.15 路径（以及仅有 sequence_validator、无实际非零 rules/windows 前缀的路径）为 **`1e-6 ≤ lo ≤ hi < stream.gap_s`**；仅当 `--limit` 后实际非零配额前缀存在生效 rules/windows 时，v1.16 联合路径才允许 **`hi ≤ stream.gap_s`**（v1.14 微秒地板——亚微秒间隔被时间戳精度取整成 0，破坏 ts 严格递增，也让词表的 0.0 边界失去意义）；`2 × max(各类 len_range 上界) ≤ stream.session_max_len`；`stream.key == []` ∧ `stream.gap_steps == 0`（定向配置错误——会话边界由交织器铺设）；`session_max_span_s > 0` ⇒ `(session_max_len − 1) × frame_gap_s 上界 ≤ session_max_span_s`；`ts_start` 须可解析为 ISO-8601
...
37. **序列规则、日历窗口与序列 hook**（v1.16）：`rules` / `windows` / `generate.sequence_validator` 仅时间流形态合法。十五个规则模板的参数矩阵启动即查；二元规则可带微秒可表示的半开 `time_s=[lo,hi)` 与顶层 required 同型字段的 `correlation={operator="equal",source_field,target_field}`，correlation 字段不得是 time_fields。窗口按帧类唯一，`of_day` 是不重叠的同日半开多窗，`of_week` 是 mon–sun 非空集合；跨午夜单分支拒收，时区固定取 `ts_start` offset。按类 rules/windows 各自采用缺键继承、空表清空、非空整表覆盖，不要求全局锚。任一非零配额类的生效规则/窗口非空时，联合 CP-SAT 在 LLM 前冻结长度、帧类词、witness、session、crossing、timestamp 与最大可行 noise 槽；每个类×生效档×每个候选长度与实际全流都须可验证。solver 固定单 worker 与 deterministic budget；`INFEASIBLE` / `UNKNOWN` 是配置错误，`MODEL_INVALID` 是内部错误。hook 每序列一次，输入 payload 为深拷贝；异常按违规作废，不激活 planner
...
| `[class.*.generate]` | instruction / styles / num_per_record / temperature；时间流形态另加 sequences / len_range、按类 `tiers`，以及 v1.16 的按类 `rules` / `windows` 三态整表 | llms、mixture、weights、seeds_per_call、num_per_call、sample_validator、sequence_validator |
...
## A.13 project.toml — [generate.stream]、帧内容与序列约束（v1.16）

`[generate.stream]` 是 generate 算子的第三形态：`generate_only` 下交付序列主输出与可重放的逐帧工件。无有效规则/窗口时保留 v1.15 的蓝图和机械交织路径；v1.16 约束面在场时，联合 planner 在任何内容调用前冻结全流 skeleton，LLM 只补 brief 与 payload。可运行样板 `examples/synth-stream` 同时演示档位、时间字段、correlation、20–40 分钟 chain 规则、工作日日历窗口与序列 hook。

| 键 | 默认 | 一句话 | 章 |
|---|---|---|---|
| `generate.stream.enabled` | false | 形态总开关；开启即触发 A.8 第 30 条的六项前提合取 | 27 |
| `generate.stream.sessions` | 0 | primary 会话数（≥ 1），须 `sessions ≤ Σsequences ≤ 2 × sessions`；完整计划有 `Σsequences − sessions` 个双 owner 会话，最终 crossed 仍按 survivor 的真实交替重算 | 27 |
...
| `generate.stream.frame_gap_s` | [5, 60] | 未被显式 timed positive witness 覆盖的 owner 相邻边使用的闭区间；默认 v1.15 路径（以及仅有 sequence_validator、无实际非零 rules/windows 前缀）为 `1µs ≤ lo ≤ hi < stream.gap_s`；仅当 `--limit` 后实际非零配额前缀存在生效 rules/windows 时，v1.16 联合路径才允许 `hi ≤ stream.gap_s` | 27 |
...
| 键 | 默认 | 一句话 | 章 |
|---|---|---|---|
| `[[generate.stream.rules]].template` | 行内必填 | 闭集：existence / absence / exactly / init / end / responded_existence / co_existence / response / precedence / succession / alternate_response / chain_response / chain_precedence / not_co_existence / not_succession | 27 |
...
| `[[generate.stream.windows]]` | [] | 每帧类至多一行；`of_day` 为同日半开多窗，`of_week` 缺省全周；固定 `ts_start` offset，无 DST | 27 |
| `[[class.<类>.generate.rules]]` / `windows` | 缺键继承 | 空数组清空本类，非空数组整表覆盖；两张表分别裁定 | 27 |
